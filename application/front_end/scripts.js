...
*/
const getList = async () => {
    let url = 'http://127.0.0.1:5000/get_products';
    fetch(url, {
      method: 'get',
    })
      .then((response) => response.json())
      .then((data) => {
        data.products.forEach(item => insertList(item.name, item.quantity, item.price))
      })
      .catch((error) => {
  ...
  */
  const postItem = async (inputProduct, inputQuantity, inputPrice) => {
    ...
    let url = 'http://127.0.0.1:5000/add_product';
    fetch(url, {
      method: 'post',
      body: formData
    })
  ...
  */
  const deleteItem = (item) => {
    console.log(item)
    let url = 'http://127.0.0.1:5000/product?name=' + item;
    fetch(url, {
      method: 'delete'
    })
      .then((response) => response.json())
